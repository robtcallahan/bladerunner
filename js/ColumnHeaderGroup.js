...
Ext.ux.grid.ColumnHeaderGroup = Ext.extend(Ext.util.Observable, {
    ...
    viewConfig: {
        ...
        setHeaderRows: function(rows) {
            this.grid.colModel.rows = rows;
            this.grid.headerRows = rows;
        },
        ...
        renderHeaders: function(){
            var ts = this.templates,
                ...
                cm = this.cm,
                //rows = cm.rows,
                rows = this.grid.headerRows || cm.rows,
            ...
            for(var row = 0, rlen = rows.length; row < rlen; row++){
                ...
                for(var i = 0, gcol = 0, len = r.length; i < len; i++){
                    ...
                    // added by Rob Callahan on 8/3/2015. Adding 2 pixels to account for a 1px border on the right and left
                    var width = parseInt(gs.width.replace(/px/, '')) + 2;
                    cells[i] = ts.gcell.apply({
                        ...
                        style: 'width:' + width + 'px;' + (gs.hidden ? 'display:none;' : '') + (group.align ? 'text-align:' + group.align + ';' : ''),
